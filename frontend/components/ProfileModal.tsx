...
export function ProfileModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const fetchResumes = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/v1/resumes`, {
  ...
  const handleRenameResume = async (id: string) => {
    ...
    try {
      const token = sessionStorage.getItem("access_token");
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/v1/resumes/${id}`, {
        method: "PATCH",
        headers: {
  ...
  const handleViewResume = async (id: string) => {
    ...
    try {
      ...
      // Generate PDF
      const genRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/v1/resumes/${id}/generate-pdf`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      // Download PDF Blob
      const downRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/v1/resumes/${id}/download`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      const token = sessionStorage.getItem("access_token");
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/v1/users/me`, {
        method: "PATCH",
        headers: { 
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}` 
  ...
  const handleChangePassword = async () => {
    ...
    try {
      const token = sessionStorage.getItem("access_token");
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/v1/users/me/change-password`, {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}` 
  ...
  const handleDeleteProfile = async () => {
    ...
    try {
      const token = sessionStorage.getItem("access_token");
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/v1/users/me`, {
        method: "DELETE",
        headers: { 
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}` 
  ...
  const handleCropComplete = async (croppedImageBlob: Blob) => {
    ...
    try {
      const token = sessionStorage.getItem("access_token");
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/v1/users/me/avatar`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        body: formData
      });
