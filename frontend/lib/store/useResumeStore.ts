        ...
        timeoutId = setTimeout(async () => {
            if (resumeId) {
                try {
                    ...
                    await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/v1/resumes/` + resumeId, {
                        method: "PATCH",
                        headers: {
        ...
        try {
            if (resumeId) {
                ...
                if (res.ok) {
                    ...
                    await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/v1/resumes/` + newResume.id, {
                        method: "PATCH",
                        headers: {
                            "Content-Type": "application/json",
                            "Authorization": `Bearer ${token}`
